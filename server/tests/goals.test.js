...
test('creates a goal and computes progress', async () => {
  ...
  assert.equal(res.body.data.goal.type, 'emergency-fund');
  assert.equal(res.body.data.goal.targetAmount, 200000);
  assert.equal(res.body.data.goal.currentAmount, 50000);
  assert.equal(res.body.data.goal.progress, 25);
